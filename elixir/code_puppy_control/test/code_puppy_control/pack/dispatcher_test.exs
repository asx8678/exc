defmodule CodePuppyControl.Pack.DispatcherTest do
  ...
  describe "dispatch/2 round-robin" do
    ...
    test "dispatch order is deterministic for the same set of workers" do
      ...
      # Release slots after each dispatch so capacity doesn't skew ordering.
      # This test is about counter determinism, not slot lifecycle.
      results1 =
        Enum.map(1..4, fn _ ->
          {:ok, node} = Dispatcher.dispatch(:terrier)
          Dispatcher.release_slot(node)
          node
        end)
      ...
      # Two batches from a clean state produce the same ordering
  ...
  describe "independent round-robin counters per sub-agent type" do
    test "terrier and watchdog rotate independently" do
      ...
      # Dispatch :terrier three times (should rotate across 3 workers).
      # Release slots so terrier capacity doesn't affect watchdog below.
      ...
      # Release terrier slots — this test is about counter independence,
      # not cross-type slot contention.
      Dispatcher.release_slot(t1)
      Dispatcher.release_slot(t2)
      Dispatcher.release_slot(t3)
    ...
    test "dispatch on one type doesn't affect another type" do
      ...
      # Release slots so terrier capacity doesn't affect watchdog workers.
      Enum.each(1..10, fn _ ->
        case Dispatcher.dispatch(:terrier) do
          {:ok, node} -> Dispatcher.release_slot(node)
          _ -> :ok
        end
      end)
  ...
  describe "edge cases" do
    test "single worker always gets the dispatch" do
      ...
      # Release after each dispatch — this test is about routing, not capacity.
      results =
        Enum.map(1..5, fn _ ->
          {:ok, node} = Dispatcher.dispatch(:terrier)
          Dispatcher.release_slot(node)
  ...
  # Slot-specific tests live in dispatcher_slot_test.exs:
  # acquire_slot/release_slot, dispatch capacity gating, worker_load,
  # active_slots, slot telemetry, status with slots.
