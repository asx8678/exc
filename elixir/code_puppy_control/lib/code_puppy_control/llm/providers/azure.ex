defmodule CodePuppyControl.LLM.Providers.Azure do
  @moduledoc """
  Azure OpenAI provider.

  Implements `CodePuppyControl.LLM.Provider` for Azure OpenAI's
  `/openai/deployments/{deployment}/chat/completions` API.
  ...
  - `:api_key` — Azure OpenAI API key (required, or `AZURE_OPENAI_API_KEY` env var)
  - `:base_url` — Azure endpoint (required, or `AZURE_OPENAI_ENDPOINT` env var,
    e.g. `"https://myresource.openai.azure.com"`)
  - `:deployment` — Deployment name (required, or `AZURE_OPENAI_DEPLOYMENT` env var)
  - `:api_version` — API version string (default: `"2024-02-15-preview"`)
  - `:model` — Model name for response metadata (default: deployment name)
  ...
  defp resolve_endpoint(opts) do
    Keyword.get(opts, :base_url) ||
      CodePuppyControl.ModelFactory.Credentials.env_or_store("AZURE_OPENAI_ENDPOINT") ||
      ""
  end

  defp resolve_deployment(opts) do
    Keyword.get(opts, :deployment) ||
      System.get_env("AZURE_OPENAI_DEPLOYMENT") ||
      "gpt-4o"
  end
  ...
  # Azure URL format: https://{resource}.openai.azure.com/openai/deployments/{deployment}/chat/completions?api-version=...
  defp build_url(endpoint, deployment, api_version) do
    normalized = String.replace_trailing(endpoint, "/", "")
    "#{normalized}/openai/deployments/#{deployment}/chat/completions?api-version=#{api_version}"
  end
  ...
  defp resolve_api_key do
    CodePuppyControl.ModelFactory.Credentials.env_or_store("AZURE_OPENAI_API_KEY") || ""
  end
end
