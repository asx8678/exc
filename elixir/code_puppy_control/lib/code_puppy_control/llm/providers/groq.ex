defmodule CodePuppyControl.LLM.Providers.Groq do
  @moduledoc """
  Groq inference provider.

  Implements `CodePuppyControl.LLM.Provider` for Groq's OpenAI-compatible
  ...
  - `:api_key` — Groq API key (required, or `GROQ_API_KEY` env var)
  ...
  defp build_url(base_url) do
    normalized = String.replace_trailing(base_url, "/openai/v1/", "")
    normalized = String.replace_trailing(normalized, "/openai/v1", "")
  ...
  defp resolve_api_key do
    CodePuppyControl.ModelFactory.Credentials.env_or_store("GROQ_API_KEY") || ""
  end
end
