...
3. **Significant plugin parity gap** — 26 of 42 Python plugins (62%) have no Elixir equivalent. Core plugins are ported; many secondary plugins are Python-only.
...
**Verdict:** Phases A–G are genuinely done. Phase H needs new infrastructure (feature flags, runtime selector) before any gradual rollout can begin. Plugin parity is a long tail but not a rollout blocker since the bridge allows Python plugins to function under Elixir control.
...
| **Browser tools** | `browser/` (4,750 LoC) | — | ❌ **Not ported** (DROP-V1 per THIN_SHELL_CONTRACT) |

### Phase F: Plugins — ✅ COMPLETE (core), ❌ PARTIAL (long tail)
...
**Ported secondary plugins (10/42):**
...
**NOT ported (26/42 — 62%):**
agent_shortcuts, agent_skills, auto_test_control, claude_code_hooks, clean_command, code_explorer, code_skeleton, completion_notifier, customizable_commands, dual_home, error_logger, example_custom_command, file_permission_handler, frontend_emitter, hook_creator, hook_manager, ollama_setup, pop_command, proactive_guidance, prompt_store, remember_last_agent, render_check, repo_compass, session_logger, shell_safety, state_migration, supervisor_review, synthetic_status, theme_switcher, tool_allowlist, tracing_langfuse, tracing_langsmith, ttsr, universal_constructor

> **Assessment:** Most unported plugins are secondary/convenience plugins. The bridge allows Python plugins to operate under Elixir control, so this is a long-tail concern, not a rollout blocker. The critical path plugins (callbacks, hook engine, loader, OAuth, pack parallelism) are all ported.

### Phase G: CLI + UI — ✅ COMPLETE

| Capability | Python | Elixir | Parity |
|------------|--------|--------|--------|
| Interactive loop / REPL | `interactive_loop.py` (28K) | `REPL.Loop` + `Dispatch` + `History` | ✅ Full |
| Slash commands | `command_line/` (58 files, 17K LoC) | `CLI.SlashCommands` (16 built-ins + dispatcher) | ✅ Full |
| MCP CLI | `command_line/mcp/` (23 files) | `CLI.SlashCommands.Commands.MCP` + `MCPLifecycle` | ✅ Full |
| TUI | `tui/` (Textual) | `TUI` (Owl — ADR-007) | ✅ Full |
...
| Capability | Spec (ADR-004) | Current State | Gap |
|------------|----------------|---------------|-----|
| Feature flags (`flags.json`) | Per-capability toggles in `~/.code_puppy_ex/flags.json` | **Does not exist** — no `FeatureFlag` module, no `flags.json` | ❌ **BLOCKER** |
| Runtime selector | `PUP_RUNTIME=python\|elixir\|auto` with fallback | Only `is_pup_ex()` binary detection + `PUP_RUNTIME=elixir` | ❌ **BLOCKER** |
| Gradual rollout | Canary → 100% per capability | No rollout infrastructure | ❌ **BLOCKER** |
| Python tree deletion | When Elixir at parity | Deferred — long tail of unported plugins | ⚠️ Deferrable |
...
3. CLI: `/flags` command (already exists as a slash command but not wired to feature flags)
...
**Recommended issue:** New issue — `code_puppy-djs.8` (Runtime selector + dual-run router)

### BLOCKER 3: Gradual Rollout Infrastructure

**What ADR-004 requires:** Canary → 100% per capability, not all-at-once.

**What exists:** Nothing. No rollout controller, no percentage-based enablement, no observability for rollout status.

**What needs to be built:**
1. Rollout controller — per-capability percentage (0% → 5% → 25% → 50% → 100%)
2. Observability — metrics on which runtime handled each request
...
|---------|----------|-------|
| 26 unported Python plugins | Medium | Bridge allows Python plugins under Elixir; not rollout-blocking |
| Browser tools (4,750 LoC) | Low | Declared DROP-V1; no Elixir browser automation ecosystem |
| 13 unported specialized agents | Low | Most are prompt-only variants; base behaviour is ported |
| TUI screen parity (3 vs 17) | Low | TUI is opt-in; CLI parity is complete |
...
2. **Create `code_puppy-djs.8`** — Runtime selector + dual-run router
...
Phases A–G are substantively complete. The Elixir codebase is mature (79K LoC, 362 test files, comprehensive supervision tree). Phase H's three blockers are all **new infrastructure** that must be built before any gradual rollout:

1. Feature-flag system (spec in ADR-004, no implementation)
2. Runtime selector with dual-run routing (spec in ADR-004, no implementation)
3. Gradual rollout controller (spec in ADR-004, no implementation)

The good news: these are self-contained, buildable features, not deep architectural gaps. The bridge already provides bidirectional Python↔Elixir communication. The bad news: until these exist, Phase H is blocked on infrastructure, not parity.
