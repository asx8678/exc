...
| F | Resolve Python `pup` entrypoint collision / version stream | ✓ Active/shipped since codepp 0.0.456 (code-puppy-1iz, code-puppy-259, code-puppy-z3g) | Python/PyPI stream is `codepp` `0.0.x` with canonical `code-puppy` and legacy `pup`; Elixir-native stream is `code_puppy_control` `0.1.x` with `./pup` escript / `code_puppy_control_*` Burrito outputs. Phased deprecation plan: `docs/release/python-pup-alias-deprecation-plan.md`. Phase 1 opt-in warning (`PUP_PUP_ALIAS_DEPRECATED=1`) shipped in codepp 0.0.456. |
