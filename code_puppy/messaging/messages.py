...
class FileEntry(BaseModel):
    ...
    size: int = Field(
        ge=-1,
        description="Size in bytes (0 for directories, -1 for unknown in fast mode)",
    )
...
class AgentResponseMessage(BaseMessage):
    ...
    was_streamed: bool = Field(
        default=False,
        description="Whether this content was already streamed to the terminal",
    )
    streamed_line_count: int = Field(
        default=0,
        description="Number of terminal lines occupied by streamed output (for erasure)",
    )
