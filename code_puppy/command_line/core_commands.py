...
def handle_api_command(command: str) -> bool:
    ...
    if subcommand == "start":
        # TODO(code_puppy-djs.2): Python FastAPI server removed — replaced by Phoenix
        emit_info(
...
def handle_wiggum_command(command: str) -> str | bool:
    ...
    Loops indefinitely until stopped with Ctrl+C.
    ...
    emit_info(
        "Wiggum will repeat this prompt indefinitely until you stop it with Ctrl+C."
    )
