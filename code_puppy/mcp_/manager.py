...
class MCPManager:
        ...
        if use_bridge:
            ...
            if bridge and bridge.is_connected():
                try:
                    ...
                    logger.debug(
                        f"Bridge delegation for list_servers failed, using local: {e}"
                    )
                except Exception as e:
    ...
    def _list_servers_via_bridge(self, bridge) -> list[ServerInfo]:
        ...
        try:
            ...
            logger.debug(
                f"list_servers via bridge returned {len(server_infos)} servers"
            )
            return server_infos
    ...
    def get_server_status(
        self, server_id: str, use_bridge: bool = True
    ) -> dict[str, Any]:
        """
        ...
        if use_bridge:
            ...
            if bridge and bridge.is_connected():
                try:
                    ...
                    logger.debug(
                        f"Bridge delegation for get_server_status failed, using local: {e}"
                    )
                except Exception as e:
    ...
    def _get_server_status_via_bridge(self, bridge, server_id: str) -> dict[str, Any]:
        ...
        try:
            result = asyncio.run(
                bridge.call_elixir_mcp("mcp.status", {"server_id": server_id})
            )
...
def get_mcp_manager() -> MCPManager:
    ...
    # First check (no lock) - fast path when already initialized
