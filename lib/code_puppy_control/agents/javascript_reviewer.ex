defmodule CodePuppyControl.Agents.JavaScriptReviewer do
  @moduledoc """
  JavaScript Reviewer — a snarky-but-helpful JavaScript review specialist.

  This agent focuses exclusively on `.js`/`.mjs`/`.cjs`/`.jsx` files,
  enforcing modern ES2023+ patterns, runtime safety, and bundle health.
  It provides actionable feedback with severity levels and focuses on
  what matters, not nitpicks.

  ## Focus Areas

    * **Async correctness** — promise chains vs async/await, error handling, cancellation
    * **Runtime safety** — null checks, prototype pollution, DOM safety
    * **Performance** — memoization, event delegation, virtual scrolling, workers
    * **Node.js specifics** — stream backpressure, worker threads, module design
    * **Browser APIs** — DOM diffing, intersection observers, service workers
    * **Security** — XSS prevention, CSRF protection, CSP adherence

  ## Tool Access

    * `cp_run_command` — run JavaScript tools (npm audit, eslint, etc.)
    * `cp_read_file` — examine JavaScript source files
  ...
  Defaults to `claude-sonnet-4-20250514` for detailed JavaScript analysis.
  """
  ...
  @impl true
  @spec name() :: :javascript_reviewer
  def name, do: :javascript_reviewer

  @impl true
  @spec display_name() :: String.t()
  def display_name, do: "JavaScript Reviewer ⚡"

  @impl true
  @spec description() :: String.t()
  def description,
    do: "Snarky-but-helpful JavaScript reviewer enforcing modern patterns and runtime sanity"
  ...
  def system_prompt(_context) do
    ...
    - Use cp_list_agents to find specialists for specific frameworks (React, Vue, Angular) or deployment concerns
  ...
  def allowed_tools do
    [
      # Shell execution for JavaScript tooling (npm audit, eslint, etc.)
      :cp_run_command,
