defmodule CodePuppyControl.Agents.GolangReviewer do
  @moduledoc """
  Golang Reviewer — a meticulous reviewer for Go pull requests with idiomatic guidance.

  Golang Reviewer focuses exclusively on `.go` files, enforcing idiomatic Go patterns,
  concurrency safety, performance optimization, and the principles of Effective Go and
  the Google Go Style Guide.

  ## Focus Areas

    * **Idiomatic Go** — Effective Go patterns, Google Go Style Guide compliance
    * **Concurrency safety** — goroutine lifecycle, channel patterns, mutex usage
    * **Performance** — escape analysis, memory pools, profiling
    * **Error handling** — wrapping, sentinel errors, panic recovery
    * **Build toolchain** — go fmt, go vet, staticcheck, golangci-lint
    * **Testing** — table-driven tests, fuzzing, benchmarks, race detection

  ## Tool Access

    * `cp_run_command` — run Go tools (go test, go vet, staticcheck, etc.)
    * `cp_read_file` — examine Go source files
  ...
  Defaults to `claude-sonnet-4-20250514` for detailed Go code analysis.
  """
  ...
  @impl true
  @spec name() :: :golang_reviewer
  def name, do: :golang_reviewer

  @impl true
  @spec display_name() :: String.t()
  def display_name, do: "Golang Reviewer 🦴"

  @impl true
  @spec description() :: String.t()
  def description, do: "Meticulous reviewer for Go pull requests with idiomatic guidance"
  ...
  def system_prompt(_context) do
    ...
    - Use cp_list_agents to discover specialists for deployment, monitoring, or domain-specific concerns
  ...
  def allowed_tools do
    [
      # Shell execution for Go tooling (go test, go vet, staticcheck, etc.)
      :cp_run_command,
