defmodule CodePuppyControl.Agents.TypeScriptReviewer do
  @moduledoc """
  TypeScript Reviewer — a hyper-picky TypeScript review specialist.

  This agent focuses exclusively on `.ts`/`.tsx`/`.mts`/`.cts` files,
  ensuring type safety, developer experience, and runtime correctness.
  It provides actionable feedback with severity levels and focuses on
  what matters, not nitpicks.

  ## Focus Areas

    * **Type system mastery** — discriminated unions, branded types, conditional types
    * **Runtime safety** — exhaustive switch statements, null/undefined handling
    * **Full-stack types** — shared contracts, tRPC, GraphQL, zod/io-ts validators
    * **Framework idioms** — React hooks, Next.js data fetching, Angular DI
    * **Performance & DX** — tree-shaking, type-only imports, lazy-loaded routes
    * **Testing** — type-safe test doubles, fixture typing, coverage

  ## Tool Access

    * `cp_run_command` — run TypeScript tools (tsc, eslint, vitest, etc.)
    * `cp_read_file` — examine TypeScript source files
    * `cp_list_files` — explore project structure
    * `cp_grep` — search for patterns, definitions, imports
    * `cp_invoke_agent` — collaborate with other specialized agents
    * `cp_list_agents` — discover available specialists

  ## Model

  Defaults to `claude-sonnet-4-20250514` for detailed TypeScript analysis.
  """
  ...
  @impl true
  @spec name() :: :typescript_reviewer
  def name, do: :typescript_reviewer

  @impl true
  @spec display_name() :: String.t()
  def display_name, do: "TypeScript Reviewer 🦾"

  @impl true
  @spec description() :: String.t()
  def description,
    do: "Hyper-picky TypeScript reviewer ensuring type safety, DX, and runtime correctness"
  ...
  def system_prompt(_context) do
    ...
    - Use cp_list_agents to discover specialists for specific frameworks (Angular, Vue, Svelte) or deployment concerns
  ...
  def allowed_tools do
    [
      # Shell execution for TypeScript tooling (tsc, eslint, vitest, etc.)
      :cp_run_command,
